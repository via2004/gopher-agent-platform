...
func NewService(jobs Repository, client Publisher, service ChatProcessor) *Service {
	return &Service{
		jobs:    jobs,
		client:  client,
		service: service,
...
func (s *Service) Process(ctx context.Context, jobID uint64) error {
	...
	result, chatErr := s.service.ReceiveAndResponse(ctx, userID, job.ConversationID, job.Content)
	if chatErr != nil {
