...
func TestRAGStoreSaveLoadReplaceAndDelete(t *testing.T) {
	...
	if err := store.Save(ctx, 42, "notes.TXT", []byte("first")); err != nil {
		t.Fatal(err)
	}
	content, filename, err := store.Load(ctx, 42)
	if err != nil || string(content) != "first" || filename != "document.txt" {
		t.Fatalf("Load() = %q, %q, %v", content, filename, err)
	}

	if err := store.Save(ctx, 42, "notes.md", []byte("second")); err != nil {
		t.Fatal(err)
	}
	content, filename, err = store.Load(ctx, 42)
	if err != nil || string(content) != "second" || filename != "document.md" {
		t.Fatalf("Load() after replace = %q, %q, %v", content, filename, err)
	}
	if _, err := os.Stat(filepath.Join(root, "42", "document.txt")); !errors.Is(err, os.ErrNotExist) {
	...
	if _, _, err := store.Load(ctx, 42); !errors.Is(err, rag.ErrDocumentNotFound) {
...
func TestRAGStoreRejectsInvalidInputs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := store.Save(context.Background(), tt.userID, tt.filename, tt.content)
...
func TestRAGStoreHonorsCanceledContext(t *testing.T) {
	...
	cancel()
	if err := store.Save(ctx, 1, "a.txt", []byte("x")); !errors.Is(err, context.Canceled) {
		t.Fatalf("Save() error = %v", err)
	}
	if _, _, err := store.Load(ctx, 1); !errors.Is(err, context.Canceled) {
		t.Fatalf("Load() error = %v", err)
	}
	if err := store.Delete(ctx, 1); !errors.Is(err, context.Canceled) {
